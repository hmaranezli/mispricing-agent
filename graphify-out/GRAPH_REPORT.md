...
## Corpus Check
- 111 files · ~103,426 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2378 nodes · 3575 edges · 172 communities (167 shown, 5 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 5 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4c2e74e1`
...
Cohesion: 0.12
Nodes (15): init_schema(), db/schema.py — SQLite şeması ve başlatma., conn(), tests/test_db.py — db/ birim testleri. aiosqlite in-memory, sıfır sunucu., log_position_close MAE/MFE + stop slippage + timing alanlarını DB'ye yazar., init_schema sonrası candidates ve positions tabloları var., Geçen aday: passed=1, veto_layer=None., Veto yiyen aday: passed=0, veto_layer ve veto_reason dolu. (+7 more)
...
Cohesion: 0.05
Nodes (58): Yeni fırsatları tarar, konsey geçenleri açar., Yeni fırsatları tarar, konsey geçenleri açar., Yeni fırsatları tarar, konsey geçenleri açar., Yeni fırsatları tarar, konsey geçenleri açar., Yeni fırsatları tarar, konsey geçenleri açar., Yeni fırsatları tarar, konsey geçenleri açar., Yeni fırsatları tarar, konsey geçenleri açar., Yeni fırsatları tarar, konsey geçenleri açar. (+50 more)
...
Cohesion: 0.13
Nodes (17): backfill(), fetch_resolved(), Kapanmış market için resolution fiyatlarını döndürür.      Returns: {"yes_exit":, Kapanmış market için resolution fiyatlarını döndürür.      Returns: {"yes_exit":, get_connection(), log_candidate(), patch_position_resolution(), bool (+9 more)
...
Cohesion: 0.15
Nodes (12): Aynı slug için açık pozisyon varsa _run_council çağrılmaz., MAX_OPEN_POSITIONS doluysa scan_edges hiç çağrılmaz., MAX_OPEN_POSITIONS doluysa scan_edges hiç çağrılmaz., MAX_OPEN_POSITIONS doluysa scan_edges hiç çağrılmaz., MAX_OPEN_POSITIONS doluysa scan_edges hiç çağrılmaz., Aynı slug için açık pozisyon varsa _run_council çağrılmaz., Aynı slug için açık pozisyon varsa _run_council çağrılmaz., Aynı slug için açık pozisyon varsa _run_council çağrılmaz. (+4 more)
...
Cohesion: 0.05
Nodes (60): arm(), disarm(), monitor/kill_switch.py — Dosya tabanlı kill switch. touch logs/KILL → durur., hard_resume(), soft_resume(), build_durum_message(), build_stats_message(), handle_command() (+52 more)
...
Cohesion: 0.10
Nodes (20): float, int, _daily_loss_usd(), Finding'i 5 katmandan geçirir. Herhangi biri düşerse None., Bugün kapanan pozisyonlardan gerçekleşen kaybı toplar., Bugün kapanan pozisyonlardan gerçekleşen kaybı toplar., Finding'i 5 katmandan geçirir. Herhangi biri düşerse None., Finding'i 5 katmandan geçirir. Herhangi biri düşerse None. (+12 more)
...
Cohesion: 0.04
Nodes (61): bool, _monitor_positions(), Açık pozisyonları izler, çıkış koşulu varsa kapatır., Açık pozisyonları izler, çıkış koşulu varsa kapatır., Açık pozisyonları izler, çıkış koşulu varsa kapatır., Açık pozisyonları izler, çıkış koşulu varsa kapatır., Açık pozisyonları izler, çıkış koşulu varsa kapatır., Açık pozisyonları izler, çıkış koşulu varsa kapatır. (+53 more)
...
Cohesion: 0.11
Nodes (21): log_position_open(), default_ws_prices(), _pass_redteam(), _pass_verify(), tests/test_main_loop.py — main_loop birim testleri. Sıfır API çağrısı., verify() taze fiyat döndürünce finding güncellenir — execute stale fiyat kullanm, ws_triggered=True iken scan+heal mantığı atlanır (main() koşullu bloğu doğrular), ws_triggered=True iken scan+heal mantığı atlanır (main() koşullu bloğu doğrular) (+13 more)
...
Cohesion: 0.18
Nodes (11): _dynamic_stop(), float, int, Zaman bazlı dinamik stop eşiği.      Giriş anında geniş tolerans (toparlama vakt, Zaman bazlı dinamik stop eşiği.      Giriş anında geniş tolerans (toparlama vakt, Erken tutuşta stop geniş — STOP_LOSS_MAX'a yakın (%30)., Vadeye yakında stop dar — STOP_LOSS_MIN'e yakın (%12)., Erken tutuşta stop geniş — STOP_LOSS_MAX'a yakın (%30). (+3 more)
...
Cohesion: 0.14
Nodes (23): tests/test_ws_prices.py — ws_prices birim testleri. Gerçek WS bağlantısı yok., get_price_event() bir asyncio.Event döndürür., get_price_event() her çağrıda aynı instance'ı döndürür., _update_cache() çağrıldığında price_event set edilir., _reset(), test_get_ask_returns_none_when_not_cached(), test_get_bid_returns_none_when_not_cached(), test_get_price_event_is_singleton() (+15 more)
...
Cohesion: 0.11
Nodes (18): _position(), int, str, Küçük kazanç (captured < PROFIT_LOCK_MIN) → erken çıkma, resolve'a kadar tut., Geçerli bir açık pozisyon kaydı döndürür., Küçük kazanç (captured < PROFIT_LOCK_MIN) → erken çıkma, resolve'a kadar tut., Küçük kazanç (captured < PROFIT_LOCK_MIN) → erken çıkma, resolve'a kadar tut., Küçük kazanç (captured < PROFIT_LOCK_MIN) → erken çıkma, resolve'a kadar tut. (+10 more)
...
Cohesion: 0.20
Nodes (13): _fetch_slug(), find_shortterm(), main(), data/shortterm.py — Kisa vadeli BTC/ETH Up/Down market bulucu. Slug'i o anki UTC, Su an ve son birkac periyodun slug'larini uretir., Su an ve son birkac periyodun slug'larini uretir., Paylaşılan session ile tek slug sorgular (find_shortterm içi)., slugs_for_now() (+5 more)
...
Cohesion: 0.18
Nodes (11): DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır., DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır., DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır., DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır., DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır., DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır., DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır., DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır. (+3 more)
...
Cohesion: 0.05
Nodes (44): execute(), _load_open_positions(), DB'deki status=open pozisyonları yükler — restart sonrası memory'yi geri dolduru, DRY_RUN flag'ine göre executor seç. Runtime'da değerlendirilir., DRY_RUN flag'ine göre executor seç. Runtime'da değerlendirilir., DRY_RUN flag'ine göre executor seç. Runtime'da değerlendirilir., DRY_RUN flag'ine göre executor seç. Runtime'da değerlendirilir., DRY_RUN flag'ine göre executor seç. Runtime'da değerlendirilir. (+36 more)

### Community 111 - "Community 111"
Cohesion: 0.11
Nodes (18): main(), council/redteam.py — KATMAN 3: Şeytan Avukatı.  "Bu işlemi neden YAPMAMALIYIZ?", Bulguya karşı şeytan avukatlığı yapar.      Args:         finding:      Scout sc, Bulguya karşı şeytan avukatlığı yapar.      Args:         finding:      Scout sc, Bulguya karşı şeytan avukatlığı yapar.      Args:         finding:      Scout sc, Bulguya karşı şeytan avukatlığı yapar.      Args:         finding:      Scout sc, Bulguya karşı şeytan avukatlığı yapar.      Args:         finding:      Scout sc, redteam() (+10 more)
...
Cohesion: 0.07
Nodes (27): _handle_ws_resolved(), WS market_resolved event'ına göre açık pozisyonu anında kapat., WS market_resolved event'ına göre açık pozisyonu anında kapat., WS market_resolved event'ına göre açık pozisyonu anında kapat., WS market_resolved event'ına göre açık pozisyonu anında kapat., WS market_resolved event'ına göre açık pozisyonu anında kapat., assets_ids eşleşmiyorsa pozisyona dokunmamalı., assets_ids eşleşmiyorsa pozisyona dokunmamalı. (+19 more)

### Community 114 - "Community 114"
Cohesion: 0.25
Nodes (8): DB'deki status=open pozisyonlar yüklenir, closed olanlar atlanır., DB'deki status=open pozisyonlar yüklenir, closed olanlar atlanır., DB'deki status=open pozisyonlar yüklenir, closed olanlar atlanır., DB'deki status=open pozisyonlar yüklenir, closed olanlar atlanır., DB'deki status=open pozisyonlar yüklenir, closed olanlar atlanır., DB'deki status=open pozisyonlar yüklenir, closed olanlar atlanır., DB'deki status=open pozisyonlar yüklenir, closed olanlar atlanır., test_load_open_positions_returns_open_ones()

### Community 115 - "Community 115"
Cohesion: 0.08
Nodes (28): _open_position(), check_exit sinyal verince pozisyon open'dan closed'a geçer., Açık pozisyon fixture'ı., check_exit sinyal verince pozisyon open'dan closed'a geçer., parse_market_window None + fetch_resolved None → market_expired ile kapatılır., parse_market_window None + fetch_resolved None → market_expired ile kapatılır., Açık pozisyon fixture'ı., Açık pozisyon fixture'ı. (+20 more)
...
Cohesion: 0.14
Nodes (13): tests/test_position.py — position/manager birim testleri. API çağrısı yok., 45s sonra stop_loss çalışır — MIN_HOLD_SECS=30'dan büyük.      45s tutulmuş, 900, 45s sonra stop_loss çalışır — MIN_HOLD_SECS=30'dan büyük.      45s tutulmuş, 900, Erken tutuşta küçük kayıp stop tetiklemez — geniş tolerans korunur.      5dk tut, Erken tutuşta küçük kayıp stop tetiklemez — geniş tolerans korunur.      5dk tut, YES pozisyon: fiyat yükselince mfe_px güncellenir., YES pozisyon: fiyat yükselince mfe_px güncellenir., MAE, fiyat toparlansa bile en kötü değerde kalır. (+5 more)
...
Cohesion: 0.33
Nodes (6): DB'deki status=open pozisyonlar yüklenir, closed olanlar atlanır., stop_loss_hit → slug failed_slugs'a eklenir (kaybeden tezi aynı pencerede tekrar, stop_loss_hit → slug failed_slugs'a eklenir (kaybeden tezi aynı pencerede tekrar, stop_loss_hit → slug failed_slugs'a eklenir (kaybeden tezi aynı pencerede tekrar, stop_loss_hit → slug failed_slugs'a eklenir (kaybeden tezi aynı pencerede tekrar, test_monitor_stop_loss_adds_to_failed_slugs()

### Community 132 - "Community 132"
Cohesion: 0.22
Nodes (9): limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır., limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır., limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır., limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır., limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır., limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır., limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır., limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır. (+1 more)
...
Cohesion: 0.29
Nodes (7): YES: büyük kâr tek döngüde değil, 2 ardışık döngüde onaylanınca çıkar.      entr, NO: HL ref'in üstüne çıkınca fair_yes > 0.52 → 'thesis_invalidated'.      fair_y, YES: büyük kâr tek döngüde değil, 2 ardışık döngüde + 3s geçince onaylanınca çık, YES: büyük kâr tek döngüde değil, 2 ardışık döngüde onaylanınca çıkar.      entr, test_check_exit_profit_needs_two_cycles_yes(), test_check_exit_thesis_invalidated_no(), test_check_exit_thesis_invalidated_yes()
...
Cohesion: 0.29
Nodes (7): NO pozisyon: mae_data_quality='estimated' (1-YES ask, gerçek NO bid değil)., NO pozisyon: mae_data_quality='estimated' (1-YES ask, gerçek NO bid değil)., YES pozisyon: mae_data_quality='rest'., Stop tetiklenince sl_trigger_px, sl_trigger_pct, first_trigger_ts set edilir., test_check_exit_no_position_mae_data_quality_estimated(), test_check_exit_stop_captures_trigger_snapshot(), test_check_exit_yes_position_mae_data_quality_rest()
...
- **Why does `scan_edges()` connect `Community 107` to `Community 1`, `Community 12`, `Community 14`, `Community 17`, `Community 18`, `Community 21`, `Community 22`, `Community 23`, `Community 163`, `Community 64`, `Community 68`, `Community 73`, `Community 74`, `Community 100`, `Community 101`, `Community 102`, `Community 106`, `Community 108`, `Community 109`, `Community 111`, `Community 120`, `Community 122`, `Community 123`?**
  _High betweenness centrality (0.088) - this node is a cross-community bridge._
- **Why does `_monitor_positions()` connect `Community 72` to `Community 131`, `Community 4`, `Community 6`, `Community 7`, `Community 135`, `Community 9`, `Community 19`, `Community 23`, `Community 152`, `Community 153`, `Community 27`, `Community 161`, `Community 37`, `Community 69`, `Community 76`, `Community 84`, `Community 87`, `Community 94`, `Community 104`, `Community 115`, `Community 118`?**
