...
## Corpus Check
- 109 files · ~98,410 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2186 nodes · 3346 edges · 158 communities (153 shown, 5 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 5 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4b10bd1c`
...
2. `_monitor_positions()` - 44 edges
3. `verify()` - 40 edges
4. `check_exit()` - 40 edges
5. `redteam()` - 37 edges
6. `_process_market()` - 36 edges
7. `_position()` - 33 edges
...
## Communities (158 total, 5 thin omitted)

### Community 0 - "Community 0"
Cohesion: 0.07
Nodes (36): main(), _parse(), _parse_resolution(), Market dict'inden YES/NO resolution fiyatlarını çıkarır., Su an ve son birkac periyodun slug'larini uretir., Su an ve son birkac periyodun slug'larini uretir., Market dict'inden YES/NO resolution fiyatlarını çıkarır., slugs_for_now() (+28 more)
...
Cohesion: 0.12
Nodes (18): init_schema(), conn(), tests/test_db.py — db/ birim testleri. aiosqlite in-memory, sıfır sunucu., log_position_close MAE/MFE + stop slippage + timing alanlarını DB'ye yazar., init_schema sonrası candidates ve positions tabloları var., positions tablosunda entry_hl_price ve exit_hl_price kolonları olmalı., positions tablosunda entry_hl_price ve exit_hl_price kolonları olmalı., Geçen aday: passed=1, veto_layer=None. (+10 more)
...
Cohesion: 0.25
Nodes (8): _monitor_positions exit anındaki HL fiyatını closed dict'e eklemeli., _monitor_positions exit anındaki HL fiyatını closed dict'e eklemeli., _monitor_positions exit anındaki HL fiyatını closed dict'e eklemeli., _monitor_positions exit anındaki HL fiyatını closed dict'e eklemeli., _monitor_positions exit anındaki HL fiyatını closed dict'e eklemeli., _monitor_positions exit anındaki HL fiyatını closed dict'e eklemeli., _monitor_positions exit anındaki HL fiyatını closed dict'e eklemeli., test_monitor_passes_exit_hl_price_to_closed_position()
...
Cohesion: 0.10
Nodes (29): _finding(), _pass_gate(), _pass_risk(), findings listesinde aynı slug iki kez varsa yalnızca bir pozisyon açılır., Konsey geçince execute() çağrılır, pozisyon open_positions'a eklenir., Konsey geçince execute() çağrılır, pozisyon open_positions'a eklenir., findings listesinde aynı slug iki kez varsa yalnızca bir pozisyon açılır., findings listesinde aynı slug iki kez varsa yalnızca bir pozisyon açılır. (+21 more)
...
Cohesion: 0.11
Nodes (19): backfill(), get_connection(), log_candidate(), patch_position_resolution(), bool, float, Path, str (+11 more)
...
Cohesion: 0.10
Nodes (22): float, _daily_loss_usd(), Yeni fırsatları tarar, konsey geçenleri açar., Yeni fırsatları tarar, konsey geçenleri açar., Yeni fırsatları tarar, konsey geçenleri açar., Yeni fırsatları tarar, konsey geçenleri açar., Yeni fırsatları tarar, konsey geçenleri açar., Yeni fırsatları tarar, konsey geçenleri açar. (+14 more)
...
Cohesion: 0.14
Nodes (16): check_exit(), Pozisyon için çıkış kararı verir.      Returns:         "max_hold_time"      — M, Pozisyon için çıkış kararı verir.      Returns:         "max_hold_time"      — M, Pozisyon için çıkış kararı verir.      Returns:         "max_hold_time"     — MA, Pozisyon için çıkış kararı verir.      Returns:         "max_hold_time"     — MA, YES: büyük kâr tek döngüde değil, 2 ardışık döngüde onaylanınca çıkar.      entr, NO: HL ref'in üstüne çıkınca fair_yes > 0.52 → 'thesis_invalidated'.      fair_y, YES: büyük kâr tek döngüde değil, 2 ardışık döngüde onaylanınca çıkar.      entr (+8 more)
...
Cohesion: 0.13
Nodes (17): int, Finding'i 5 katmandan geçirir. Herhangi biri düşerse None., Finding'i 5 katmandan geçirir. Herhangi biri düşerse None., Finding'i 5 katmandan geçirir. Herhangi biri düşerse None., Finding'i 5 katmandan geçirir. Herhangi biri düşerse None., Finding'i 5 katmandan geçirir. Herhangi biri düşerse None., Finding'i 5 katmandan geçirir. Herhangi biri düşerse None., Finding'i 5 katmandan geçirir. Herhangi biri düşerse None. (+9 more)

### Community 63 - "Community 63"
Cohesion: 0.11
Nodes (19): Heal başarılıysa notify_resolved_late doğru asset/seq_no ile çağrılır., DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır., Heal başarılıysa notify_resolved_late doğru asset/seq_no ile çağrılır., DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır., DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır., Heal başarılıysa notify_resolved_late doğru asset/seq_no ile çağrılır., DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır., DRY_RUN=False iken _monitor_positions çıkışta sell_position çağırır. (+11 more)
...
Cohesion: 0.10
Nodes (27): fetch_by_slug(), fetch_resolved(), _fetch_slug(), parse_market_window(), _parse_token_ids(), str, data/shortterm.py — Kisa vadeli BTC/ETH Up/Down market bulucu. Slug'i o anki UTC, Tek slug için bağımsız sorgu — main_loop._monitor_positions kullanır. (+19 more)
...
Cohesion: 0.08
Nodes (24): _monitor_positions(), Açık pozisyonları izler, çıkış koşulu varsa kapatır., Açık pozisyonları izler, çıkış koşulu varsa kapatır., Açık pozisyonları izler, çıkış koşulu varsa kapatır., Açık pozisyonları izler, çıkış koşulu varsa kapatır., Açık pozisyonları izler, çıkış koşulu varsa kapatır., Açık pozisyonları izler, çıkış koşulu varsa kapatır., Açık pozisyonları izler, çıkış koşulu varsa kapatır. (+16 more)

### Community 73 - "Community 73"
Cohesion: 0.11
Nodes (19): DB'den yüklenen pozisyonda entry_hl_price alanı olmalı., limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır., DB'den yüklenen pozisyonda entry_hl_price alanı olmalı., DRY_RUN=False iken sell_position None → pozisyon AÇIK kalır, kapatılmaz.      FA, DRY_RUN=False iken sell_position None → pozisyon AÇIK kalır, kapatılmaz.      FA, DB'den yüklenen pozisyonda entry_hl_price alanı olmalı., _heal entry_hl_price DB'den okuyup exit için current_price çağırmalı., DB'den yüklenen pozisyonda entry_hl_price alanı olmalı. (+11 more)
...
Cohesion: 0.11
Nodes (22): log_position_open(), default_ws_prices(), _pos_with_token(), tests/test_main_loop.py — main_loop birim testleri. Sıfır API çağrısı., yes_token_id + no_token_id içeren açık pozisyon fixture'ı., YES + WS bid mevcut → price_source='ws_bid', mae_data_quality='exact'., YES + WS bid None → CLOB REST fallback → price_source='clob_rest_bid', mae_data_, NO + WS ask mevcut → price_source='ws_ask_complement', mae_data_quality='estimat (+14 more)
...
Cohesion: 0.25
Nodes (8): _dynamic_stop(), float, int, Zaman bazlı dinamik stop eşiği.      Giriş anında geniş tolerans (toparlama vakt, Erken tutuşta stop geniş — STOP_LOSS_MAX'a yakın (%30)., Vadeye yakında stop dar — STOP_LOSS_MIN'e yakın (%12)., test_dynamic_stop_tighter_near_expiry(), test_dynamic_stop_wider_early()
...
Cohesion: 0.07
Nodes (29): DB'deki status=open pozisyonlar yüklenir, closed olanlar atlanır., DB'deki status=open pozisyonlar yüklenir, closed olanlar atlanır., DRY_RUN YES çıkışta pm_exit_price = best_bid (satış fiyatı), best_ask değil., DRY_RUN=True → _run_council'a daily_loss_usd=0 geçilir, kayıp limiti uygulanmaz., fetch_resolved hâlâ None dönerse DB kaydına dokunulmaz., fetch_resolved hâlâ None dönerse DB kaydına dokunulmaz., DB'deki status=open pozisyonlar yüklenir, closed olanlar atlanır., DB'deki status=open pozisyonlar yüklenir, closed olanlar atlanır. (+21 more)
...
Cohesion: 0.10
Nodes (20): Regression: n_open_before'un _monitor_positions'dan ÖNCE alınması,     aynı turd, BANKROLL_USD env değişkeni set edilince main_loop bu değeri kullanır., Regression: n_open_before'un _monitor_positions'dan ÖNCE alınması,     aynı turd, Regression: n_open_before'un _monitor_positions'dan ÖNCE alınması,     aynı turd, BANKROLL_USD env değişkeni set edilince main_loop bu değeri kullanır., Regression: n_open_before'un _monitor_positions'dan ÖNCE alınması,     aynı turd, BANKROLL_USD env değişkeni set edilince main_loop bu değeri kullanır., BANKROLL_USD env değişkeni set edilince main_loop bu değeri kullanır. (+12 more)
...
Cohesion: 0.12
Nodes (17): limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır., limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır., limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır., limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır., fetch_resolved None döndürürse notify_resolved_late çağrılmaz., fetch_resolved None döndürürse notify_resolved_late çağrılmaz., fetch_resolved None döndürürse notify_resolved_late çağrılmaz., limit=2 → 5 null kayıt varsa sadece 2 işlenir, 3 null kalır. (+9 more)
...
Cohesion: 0.14
Nodes (18): bool, run_canary(), bool, run_test(), ClobClient, find_shortterm(), get_effective_bankroll(), float (+10 more)

### Community 94 - "Community 94"
Cohesion: 0.09
Nodes (22): _heal_pending_resolutions(), market_expired + pm_exit_price=None kayıtları için resolution retry eder., market_expired + pm_exit_price=None kayıtları için resolution retry eder., market_expired + pm_exit_price=None kayıtları için resolution retry eder., market_expired + pm_exit_price=None kayıtları için resolution retry eder., market_expired + pm_exit_price=None kayıtları için resolution retry eder., market_expired + pm_exit_price=None kayıtları için resolution retry eder., market_expired + pm_exit_price=None kayıtları için resolution retry eder. (+14 more)
...
Cohesion: 0.12
Nodes (15): tests/test_position.py — position/manager birim testleri. API çağrısı yok., Kritik gamma trap testi: 13dk tutulmuş, 2dk kaldı.      Dinamik stop ~%14.4 → eş, Erken tutuşta küçük kayıp stop tetiklemez — geniş tolerans korunur.      5dk tut, YES pozisyon: fiyat düşünce mae_px güncellenir., MFE, fiyat düşse bile en iyi değerde kalır., YES pozisyon: mae_data_quality='rest'., İkinci stop tetiklemesinde first_trigger_ts değişmez (setdefault)., time_to_expiry_secs < 90, fiyat OK → None (profit_target/max_hold engellenir). (+7 more)
...
Cohesion: 0.08
Nodes (25): _handle_ws_resolved(), WS market_resolved event'ına göre açık pozisyonu anında kapat., WS market_resolved event'ına göre açık pozisyonu anında kapat., WS market_resolved event'ına göre açık pozisyonu anında kapat., WS market_resolved event'ına göre açık pozisyonu anında kapat., assets_ids eşleşmiyorsa pozisyona dokunmamalı., assets_ids eşleşmiyorsa pozisyona dokunmamalı., YES pozisyon + 'Yes' kazanınca pm_exit=1.0 ile kapanmalı. (+17 more)
...
Cohesion: 0.10
Nodes (21): _open_position(), Açık pozisyon fixture'ı., parse_market_window None + fetch_resolved None → market_expired ile kapatılır., parse_market_window None + fetch_resolved None → market_expired ile kapatılır., window=None iken fetch_resolved sonuç verirse pm_exit_price dolu kapanır (YES)., Açık pozisyon fixture'ı., Açık pozisyon fixture'ı., Açık pozisyon fixture'ı. (+13 more)
...
Cohesion: 0.29
Nodes (7): DB'de açık pozisyon yoksa boş liste döner., DB'de açık pozisyon yoksa boş liste döner., DB'de açık pozisyon yoksa boş liste döner., DB'de açık pozisyon yoksa boş liste döner., DB'de açık pozisyon yoksa boş liste döner., DB'de açık pozisyon yoksa boş liste döner., test_load_open_positions_empty_db()
...
Cohesion: 0.11
Nodes (19): _position(), int, str, Kâr sinyali ardışık değilse sayaç sıfırlanır → tek seferlik spike çıkış yapmaz., Kâr sinyali ardışık değilse sayaç sıfırlanır → tek seferlik spike çıkış yapmaz., Kâr sinyali ardışık değilse sayaç sıfırlanır → tek seferlik spike çıkış yapmaz., Geçerli bir açık pozisyon kaydı döndürür., İlk 60s içinde stop_loss çalışmaz — anlık ters dönüş gürültüsü filtresi. (+11 more)
...
Cohesion: 0.33
Nodes (6): fetch_by_slug=None ama fetch_resolved sonuç döndürünce → market_resolved kapatıl, fetch_by_slug=None ama fetch_resolved sonuç döndürünce → market_resolved kapatıl, fetch_by_slug=None ama fetch_resolved sonuç döndürünce → market_resolved kapatıl, fetch_by_slug=None ama fetch_resolved sonuç döndürünce → market_resolved kapatıl, fetch_by_slug=None ama fetch_resolved sonuç döndürünce → market_resolved kapatıl, test_monitor_closes_on_definitive_resolution()
...
## Knowledge Gaps
- **335 isolated node(s):** `restart.sh script`, `str`, `int`, `float`, `bool` (+330 more)
...
- **Why does `scan_edges()` connect `Community 107` to `Community 1`, `Community 12`, `Community 14`, `Community 17`, `Community 18`, `Community 22`, `Community 23`, `Community 33`, `Community 163`, `Community 64`, `Community 68`, `Community 74`, `Community 93`, `Community 95`, `Community 100`, `Community 101`, `Community 102`, `Community 106`, `Community 108`, `Community 109`, `Community 111`, `Community 112`, `Community 122`, `Community 123`?**
  _High betweenness centrality (0.093) - this node is a cross-community bridge._
- **Why does `_process_market()` connect `Community 17` to `Community 64`, `Community 1`, `Community 4`, `Community 5`, `Community 102`, `Community 7`, `Community 8`, `Community 69`, `Community 107`, `Community 109`, `Community 144`, `Community 148`, `Community 22`, `Community 154`, `Community 157`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Why does `check_exit()` connect `Community 37` to `Community 96`, `Community 97`, `Community 5`, `Community 166`, `Community 103`, `Community 72`, `Community 9`, `Community 79`, `Community 146`, `Community 149`, `Community 150`, `Community 23`, `Community 151`, `Community 124`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
