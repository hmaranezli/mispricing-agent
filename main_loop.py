...
async def _scan_and_execute(
    ...
    for finding in findings:
        ...
        # ── E11f FILL-TO-SUBMIT GATE: çok submit'e karşı az gerçek açılış (düşük oran) → yeni giriş YOK ──
        # ENTRY-ONLY; in-memory opened (E10c) / submitted (E11e) sayaçları (DB COUNT YOK); exit ETKİLENMEZ.
        if fill_to_submit_halt(_session_trade_count(), _session_submit_count()) == "fill_to_submit_stop":
            print(f"[risk_gate] {slug} council GEÇTİ ama FILL_TO_SUBMIT oranı eşik altında — yeni entry engellendi")
            continue
