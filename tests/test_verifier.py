...
import config
from council.verifier import verify, PRICE_DRIFT_HALT_PCT
...
def test_soft_fail_halt_is_false():
    """Soft fail'de (fetch_error) halt=False."""
    from council.verifier import _result
    for reason in ("fetch_error",):
...
async def test_verify_real_scout_finding_has_valid_structure():
    ...
    assert result["reason"] in {"ok", "api_mismatch", "fetch_error"}
...
async def test_verify_passes_through_scout_clob_prices():
    ...
    finding = _fake_finding_with_tokens(
        action="YES", best_ask=0.55, best_bid=0.45,
    ...
    assert abs(result["fresh_best_ask"] - 0.55) < 1e-6, \
        f"Scout'un CLOB fiyatı geçirilmeli (0.55), got {result['fresh_best_ask']}"
    assert abs(result["fresh_best_bid"] - 0.45) < 1e-6
    assert abs(result["fresh_seconds"] - 300.0) < 1e-6
    assert abs(result["fresh_edge"] - 0.25) < 1e-4  # finding["edge"]=0.25 pass-through
...
async def test_verify_no_clob_api_call():
    ...
    with patch("council.verifier.current_price", new_callable=AsyncMock, return_value=105_000.0):
        # If verifier tried to import and call get_clob_price, this test structure
        # would need to handle it. The fact it passes without patching get_clob_price
        # proves verifier no longer calls it.
        result = await verify(finding)
    assert result["pass"] is True
